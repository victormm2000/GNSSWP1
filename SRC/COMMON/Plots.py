...
def generatebarPlot(PlotConf):
    ...
    for Label in PlotConf["yData"].keys():
            ax.bar(PlotConf["xData"][Label], PlotConf["yData"][Label]) # mirar variables de bar
            

    saveFigure(fig, PlotConf["Path"])

"""
EJEMPLO PARA HACER DEBUGGING INSITE
    for Label in PlotConf["yData"].keys():
        x_data = PlotConf["xData"].get(Label, [])
        x_data = pd.to_numeric(x_data, errors='coerce')  # 'coerce' turns invalid values into NaN
        y_data = PlotConf["yData"][Label]
        print(f"Label: {Label}")
        ...
        print(type(x_data), type(y_data))
         # Check if the lengths of x_data and y_data match
        if len(x_data) == len(y_data):
            ax.bar(x_data, y_data, PlotConf["Marker"], linewidth=LineWidth)
        else:
            print(f"Warning: Shape mismatch for {Label}, x_data and y_data have different lengths!")   

"""
def generatePlot(PlotConf):
    if(PlotConf["Type"] == "Lines"):
        generateLinesPlot(PlotConf)
    if (PlotConf["Type"] == "bar"):
        generatebarPlot(PlotConf)
